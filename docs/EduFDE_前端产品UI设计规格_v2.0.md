...
- 阶段一主页：模式选择与进度总览。
- 教学引导模式：六关卡训练，适合首次学习。
- 项目实战模式：客户拜访，围绕 AI 客户展开访谈。
- 拜访间整理：把访谈信息转化为结构化线索。
- 问题发现总结：形成可进入阶段二的问题定义。
- 阶段一综合评估：AI 与规则共同反馈需求理解质量。

关键设计：

- 教学引导模式和项目实战模式是两个独立页面，核心都是客户对话窗口。
- 对话窗口顶部必须展示客户身份介绍，例如厂长、车间主任、质检主管等。
- 阶段一主页左侧五阶段导航必须与现有课程实验进入后的“五阶段交付主线”导航保持一致：白色卡片、当前阶段深色高亮、完成 / 锁定状态明确。
- 教学引导模式和项目实战模式进入后使用专注任务布局，不再显示阶段一主页的五阶段交付主线和通用右侧上下文栏；教学引导页严格为左侧六关卡进度、中间客户对话、右侧本关目标与提问辅助。
- 教学引导模式左侧展示六关卡进度，右侧展示当前关卡目标、提问技巧、推荐问句和 AI 反馈区。
- 项目实战模式不展示教学技巧，右侧展示实时访谈线索、待追问问题和正式产出流程。
- 教学引导推荐首次学习完成，但不强制阻塞项目实战。
- 拜访间整理、问题发现总结和阶段一综合评估属于项目实战模式的正式交付链路；阶段二正式输入只来自项目实战模式。
- 明确区分“教学引导模式”和“项目实战模式”。
