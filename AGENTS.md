...
当前项目已完成 **MVP 本地演示闭环与收口审查**，并已完成正式学生端 UI 静态原型确认和前端产品 UI 设计规格沉淀。下一阶段重点是把现有联调页能力迁移为正式学生端产品 UI。
...
1. 正式学生端产品 UI 第一轮：应用壳、登录页、课程列表、实验项目框架、五阶段导航、中文术语映射。
2. 阶段一正式产品页面。
3. 阶段二正式产品页面。
4. 阶段三正式产品页面。
5. 阶段四正式产品页面。
6. 阶段五正式产品页面与最终项目档案袋。
7. 正式教师后台 UI。
8. 课程成员模型替换 `created_by_user_id` 临时权限边界。
9. 黄灯债务可视化。
10. 真实 AI Provider 与真实 Dify 集成。
...
- 当前已具备：学生端五阶段最小流程、Artifact 沉淀、AI Gateway fake provider、教师进度视图、基础学习画像、本地 demo seed 与启动文档。
- 当前仍非正式产品 UI，前端主要是联调工作台；教师权限仍暂以 `courses.created_by_user_id` 作为课程读取边界。
- 下一步推荐任务：正式学生端产品 UI 第一轮实现，先做应用壳、登录页、课程列表、实验项目框架、五阶段导航和中文术语映射。
